...
The `spin`, `pulse`, `bounce`, `shake`, `flip`, and `wobble` props apply CSS modifier classes (`abs-spin`, `abs-pulse`, `abs-bounce`, `abs-shake`, `abs-flip`, `abs-wobble`). These require the AbscomFonts CSS to be loaded. See [animations.md](./animations.md) for setup.
